...
import torch
import time
from models import PreActResNet18_cifar100
from models import PreActResNet18_silu_cifar100
import numpy as np
import matplotlib
...
parser = argparse.ArgumentParser(description='Adversarial Training for CIFAR100', formatter_class=argparse.RawTextHelpFormatter)
parser.add_argument("--gpu_id", help="Id of GPU to be used", type=int, default = 0)
parser.add_argument("--model", help="Type of Adversarial Training: \n\t 0: l_inf \n\t 1: l_1 \n\t 2: l_2 \n\t 3: msd \n\t 4: triple \n\t 5: worst \n\t 6: vanilla \n\t 7: mix \n\t 8: sat \n\t 9: adt", type=int, default = 7)
...
ln = args.label_noise
if args.silu:
    model = PreActResNet18_silu_cifar100().cuda()
else:
    model = PreActResNet18_cifar100().cuda()
...
if args.swa:
    if args.silu:
        swa_model = PreActResNet18_silu_cifar100().cuda()
    else:
        swa_model = PreActResNet18_cifar100().cuda()
...
model_dir = "Final_CIFAR_100/{0}".format(exp_name)
import os
